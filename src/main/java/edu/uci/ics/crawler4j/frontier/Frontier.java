...
public class Frontier extends Configurable {
  protected static final Logger logger = LoggerFactory.getLogger(edu.uci.ics.crawler4j.frontier.Frontier.class);
  ...
  protected edu.uci.ics.crawler4j.frontier.Counters counters;

  public Frontier(Environment env, CrawlConfig config) {
    super(config);
    this.counters = new edu.uci.ics.crawler4j.frontier.Counters(env, config);
    try {
      workQueues = new WorkQueues(env, DATABASE_NAME, config.isResumableCrawling());
      if (config.isResumableCrawling()) {
        scheduledPages = counters.getValue(edu.uci.ics.crawler4j.frontier.Counters.ReservedCounterNames.SCHEDULED_PAGES);
  ...
  public void scheduleAll(List<WebURL> urls) {
    ...
    synchronized (mutex) {
      ...
      if (newScheduledPage > 0) {
        ...
        counters.increment(edu.uci.ics.crawler4j.frontier.Counters.ReservedCounterNames.SCHEDULED_PAGES, newScheduledPage);
      }
    ...
    synchronized (mutex) {
      try {
        if (maxPagesToFetch < 0 || scheduledPages < maxPagesToFetch) {
          workQueues.put(url);
          scheduledPages++;
          counters.increment(edu.uci.ics.crawler4j.frontier.Counters.ReservedCounterNames.SCHEDULED_PAGES);
        }
      } catch (DatabaseException e) {
        logger.error("Error while putting the url in the work queue", e);
      }
  ...
  public void setProcessed(WebURL webURL) {
    counters.increment(edu.uci.ics.crawler4j.frontier.Counters.ReservedCounterNames.PROCESSED_PAGES);
    if (inProcessPages != null) {
